...
echo "$($PSQL "SELECT name FROM (SELECT winner_id AS team_id FROM games WHERE year = '2014' AND round = 'Eighth-Final' UNION SELECT opponent_id AS team_id FROM games WHERE year = '2014' AND round = 'Eighth-Final') AS participating_teams LEFT JOIN teams USING(team_id) ORDER BY name")"
